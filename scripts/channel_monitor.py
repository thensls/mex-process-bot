...
def is_question(text, api_key):
    ...
    system = (
        ...
        "\n\n"
        "IMPLICIT ASKS — also return is_question: true for messages that describe a member "
        "situation where the member is requesting/wants/asks/is inquiring about something. "
        "Even without an explicit question word, these are implicit asks for guidance from "
        "the MEX team about how to handle the request. Examples that should return true:\n"
        "  - 'The member is requesting a refund for X' (implicit: how should I handle this?)\n"
        "  - 'She is requesting expedited shipping' (implicit: can we accommodate?)\n"
        "  - 'He wanted to know whether it would be possible to receive a refund'\n"
        "  - 'Member called because [situation]. She is requesting [thing].'\n"
        "  - 'The member called to inquire about X'\n"
        "  - 'Member reached out asking for Y'\n"
        "  - Any message describing a member-facing situation with an implied need for "
        "guidance from leadership or the SOP.\n"
        "\n"
        "Return is_question: false ONLY for: general announcements, greetings, celebrations, "
        "status updates that don't describe a member request, acknowledgements ('thanks!'), "
        "chitchat, reactions, FYI updates with no implied need for guidance, or messages "
        "with no ask at all."
    )
    try:
